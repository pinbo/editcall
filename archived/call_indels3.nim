...
proc writeHelp() =
  # quit("synopsis: " & getAppFilename() & " input.sam -g=reference.fa -o=output.txt")
  quit("synopsis: ./editcall input.sam reference.fa output.txt")
# proc writeVersion() =
...
## use regular positioned based arguments
let
  samFile = commandLineParams()[0]
  refFile = commandLineParams()[1]
  outfile = commandLineParams()[2]
  ...
  if line[0] == '@':
    ...
    if ('I' in cigar or 'D' in cigar or 'N' in cigar) or ("SA:Z" in line and 'H' notin cigar):
      ...
      for i in 0 .. ss1.len-1:
        ...
        if ss1[i] == 'M' or ss1[i] == '=' or ss1[i] == 'X':
          ...
          kk = @[chrom, $refPos, $(refPos+1), refSeq, altSeq, $mutSize].join("\t")
          if kk in indelDict:
            indelDict[kk] += 1
          else:
            indelDict[kk] = 1
          ...
          mutSize = altSeq.len - refSeq.len
          kk = @[chrom, $refPos, $(refPos+num+1), refSeq, altSeq, $mutSize].join("\t")
          if kk in indelDict:
            indelDict[kk] += 1
          else:
            indelDict[kk] = 1
    ...
    if "SA:Z" in line and 'H' notin cigar: # only when the read has no hard clip, eg. read is raw
      ...
      if chrom == saChrom and strand == saStrand:# and (flag and 0x20) == 0: # big deletion, could be insertion too, but update later
        ...
        mutSize = altSeq.len - refSeq.len
        kk = @[chrom, $(refPos1+1), $delEndPos, refSeq, altSeq, $mutSize].join("\t")
        if kk in indelDict:
          indelDict[kk] += 1
        else:
          indelDict[kk] = 1
...
f.writeLine("Chrom\trefStartPos\trefEndPos\tRef\tAlt\tmutSize\tnMutRead")
