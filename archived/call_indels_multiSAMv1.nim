    ...
    if line[0] == '@':
      ...
      if "SA:Z" in line and 'H' notin cigar: # only when the read has no hard clip, eg. read is raw
        ...
        if chrom == saChrom and strand == saStrand:# and (flag and 0x20) == 0: # big deletion, could be insertion too, but update later
          ...
          mutSize = altSeq.len - refSeq.len
          kk = @[samFile, chrom, $(refPos1+1), $delEndPos, refSeq, altSeq, $mutSize].join("\t")
          if kk in indelDict:
            indelDict[kk] += 1
          else:
            indelDict[kk] = 1
          ...
          var
            ...
            readPos2 = allPos2[1]
            refPos2  = allPos2[3]
          if (flag and 0x80) != 0: # 2nd in pair, need to count from right. This is only for my CRISPR NGS input, not correct for others
            readPos1 = allPos2[0]
            refPos1  = allPos1[2]
            readPos2 = allPos1[0]
            refPos2  = allPos2[2]
